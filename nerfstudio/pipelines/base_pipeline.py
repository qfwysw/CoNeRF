...
class VanillaPipeline(Pipeline):
    ...
    def __init__(
        ...
        self.obb_center = (0., 0., -0.3)
        self.obb_rotation = (0., 0., 0.)
        self.obb_scale = (0.8, 0.8, 0.8)
    ...
    def get_average_image_metrics(
        ...
        with Progress(
            ...
            for camera, batch in data_loader:
                ...
                inner_start = time()
                outputs = self.model.get_outputs_for_camera(camera=camera, obb_box=self.crop_obb)
                # outputs = self.model.get_outputs_for_camera(camera=camera, obb_box=None)
                height, width = camera.height, camera.width
                num_rays = height * width
                metrics_dict, image_dict = self.model.get_image_metrics_and_images(outputs, batch, idx)
                # import pdb;pdb.set_trace()
                # depth = image_dict['depth']
                # print(depth)
                # import pdb;pdb.set_trace()
                if output_path is not None:
                    for key in ['img', 'accumulation', 'depth']:
