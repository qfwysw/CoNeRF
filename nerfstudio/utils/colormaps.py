...
def apply_colormap(
    ...
    if image.shape[-1] == 1 and torch.is_floating_point(image):
        ...
        if colormap_options.invert:
            output = 1 - output
        return apply_float_colormap(output, colormap='gray')
...
def remove_statistical_outliers(accumulation, z_thresh=3.0):
    """
    移除accumulation中数值偏离均值太大的离群点

    参数:
    - accumulation: torch.Tensor, shape (H, W)
    - z_thresh: float, Z-score 阈值，超过这个值的点会被置为0

    返回:
    - filtered_accumulation: torch.Tensor
    """
    # 只统计非零值
    nonzero_values = accumulation[accumulation != 0]
    
    if nonzero_values.numel() == 0:
        return accumulation  # 没有非零值，直接返回

    mean = nonzero_values.mean()
    std = nonzero_values.std()
    import pdb;pdb.set_trace()
    # 计算每个像素的z-score（仅对非零像素）
    z_scores = torch.zeros_like(accumulation)
    z_scores[accumulation != 0] = torch.abs((accumulation[accumulation != 0] - mean) / std)

    # 创建一个掩码：z-score 超过阈值的认为是离群点
    outlier_mask = z_scores > z_thresh

    # 清除离群点
    filtered_accumulation = accumulation.clone()
    filtered_accumulation[outlier_mask] = 0

    return filtered_accumulation
...
def apply_depth_colormap(
    ...
    # gamma = 1
    # depth_gamma = torch.pow(depth, gamma)
    # depth_enhanced = torch.sigmoid((depth_gamma - 0.5) * 100)
...
#     accumulation: Optional[Float[torch.Tensor, "*bs 1"]] = None,
#     near_plane: Optional[float] = None,
#     far_plane: Optional[float] = None,
#     colormap_options: ColormapOptions = ColormapOptions(),
#     depth_mask: Optional[Bool[torch.Tensor, "*bs 1"]] = None
# ) -> Float[torch.Tensor, "*bs rgb=3"]:
#     """Converts a depth image to color for easier analysis.
...
#     # import pdb;pdb.set_trace()
#     # depth_normalized.max()
#     # Step 2: Mask
#     # depth_mask = (depth_normalized >= 0.03).squeeze(-1)

#     # Step 3: Enhance contrast
#     gamma = 0.1
#     depth_gamma = torch.pow(depth_normalized, gamma)
#     depth_enhanced = torch.sigmoid((depth_gamma - 0.5) * 20)

#     # Step 4: Colormap
#     colored_image = apply_colormap(depth_enhanced, colormap_options=colormap_options)

#     # Step 5: Accumulation blending
#     if accumulation is not None:
#         accumulation = accumulation.clone()
#         # accumulation[depth_mask] = 0
#         # accumulation = remove_statistical_outliers(accumulation, z_thresh=2.0)
#         colored_image = colored_image * accumulation + (1 - accumulation)
...
#     # Step 2: Mask
#     depth_mask = (depth_normalized >= 0.0015).squeeze(-1)
...
#     depth = (depth - near_plane) / (far_plane - near_plane + 1e-10)
#     depth = torch.clip(depth, 0, 1)
#     depth_mask =  (depth >= 0.001)
#     # print(depth.min(), depth.max(), depth.sum())
...
#         accumulation[depth_mask] = 0
#         colored_image = colored_image * accumulation + (1 - accumulation)
#         # print(colored_image.min(), colored_image.max(), colored_image.sum())
#         # import pdb;pdb.set_trace()
#     # colored_image[depth_mask] = 0
#     return colored_image, depth_mask
