...
class EdgeSharpMask(nn.Module):
    ...
    def forward(self, pred, gt, batch):
        ...
        size_len = 4096 // len(batch_ids) // 3 * 3
        # pred 和 gt 输入为 [N, 3]
        pred = pred.transpose(0, 1).unsqueeze(0)  # [1, 3, N]
        gt = gt.transpose(0, 1).unsqueeze(0)      # [1, 3, N]

        # 对每个通道分别应用 Laplacian
        edge_pred_r = self._laplacian_1d(pred[:, 0:1, :])
        edge_pred_g = self._laplacian_1d(pred[:, 1:2, :])
        edge_pred_b = self._laplacian_1d(pred[:, 2:3, :])
        edge_pred_max = torch.stack((edge_pred_r, edge_pred_g, edge_pred_b), dim=-1)  # [1, 1, N]
        # edge_pred_max *= 100
        # edge_pred_max  = edge_pred_max.max(dim=-1)[0]

        edge_gt_r = self._laplacian_1d(gt[:, 0:1, :])
        edge_gt_g = self._laplacian_1d(gt[:, 1:2, :])
        edge_gt_b = self._laplacian_1d(gt[:, 2:3, :])
        edge_gt_max = torch.stack((edge_gt_r, edge_gt_g, edge_gt_b), dim=-1)  # [1, 1, N]
        # edge_gt_max *= 100
        # import pdb;pdb.set_trace()
        # edge_gt_max  = edge_gt_max.max(dim=-1)[0]
        if self.use_mask:
            # 使用掩码时，仅在非背景区域计算损失
            # 背景判定方式可调整
            background_mask = (gt != 1.0).any(dim=1, keepdim=True)  # 非背景为 True
            fused_loss = self.mse_loss(edge_pred_max * background_mask, edge_gt_max * background_mask)
        else:
            # import pdb;pdb.set_trace()
            fused_loss = self.mse_loss(edge_pred_max, edge_gt_max)
        # import pdb;pdb.set_trace()
        self.count += 1
        return fused_loss
