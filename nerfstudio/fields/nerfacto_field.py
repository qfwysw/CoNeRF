...
class NerfactoField(Field):
    ...
    def get_density(self, ray_samples: RaySamples, freq_mask: tuple=None) -> Tuple[Tensor, Tensor]:
        ...
        #     encoded_xyz = positions.view(-1, 3)
        #     # import pdb;pdb.set_trace()
        #     h = self.mlp_base(encoded_xyz).view(*ray_samples.frustums.shape, -1) * dir_freq_mask
        #     # import pdb;pdb.set_trace()
        # else:
        #     encoded_xyz = positions.view(-1, 3)
        #     h = self.mlp_base(encoded_xyz).view(*ray_samples.frustums.shape, -1)
    ...
    def get_outputs(
        self, ray_samples: RaySamples, density_embedding: Optional[Tensor] = None, freq_mask: tuple=None
        ...
        if self.use_pred_normals:
            ...
            # if freq_mask is not None:
            #     (positions_freq_mask, dir_freq_mask) = freq_mask
            #     positions_flat = positions_flat * positions_freq_mask.view(1,-1)
    ...
    def forward(self, ray_samples: RaySamples, compute_normals: bool = False, freq_mask: tuple=None) -> Dict[FieldHeadNames, Tensor]:
