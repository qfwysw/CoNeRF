...
class PixelSampler:
    ...
    def rejection_sample_mask(
        ...
        """
        # print(self.config.all_step, self.config.step_ratio, self.config.sample_ratio, self.config.all_step*self.config.step_ratio)
        if self.count >= int(self.config.all_step*self.config.step_ratio):
            indices = (
                torch.rand((num_samples, 3), device=device)
                * torch.tensor([num_images, image_height, image_width], device=device)
            ).long()
            rate = self.config.sample_ratio
            # rate = 0.
            num_valid = 0
    ...
    def sample_method(
        ...
        if True:
            if self.config.rejection_sample_mask:
                ...
                # print("rejection_sample_mask", indices.shape)
            else:
    ...
    def collate_image_dataset_batch(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        # collated_batch["mask"] = batch["mask"]  # with the abs camera indices
        if keep_full_image:
            collated_batch["full_image"] = batch["image"]

        return collated_batch

    def collate_image_dataset_batch_list(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False, step: int=None):
        """
        ...
        # print(num_rays_per_batch, num_images)
        if "mask" in batch:
            for i, num_rays in enumerate(num_rays_per_image):
                ...
                all_maskes.append(batch["mask"][i][indices[:, 1], indices[:, 2]])
                if "depth_image" in batch:
                    all_depth_images.append(batch["depth_image"][i][indices[:, 1], indices[:, 2]])

        else:
            for i, num_rays in enumerate(num_rays_per_image):
                # print("num_rays: ", num_rays, len(batch["image"]))
                image_height, image_width, _ = batch["image"][i].shape
                if self.config.is_equirectangular:
                    indices = self.sample_method_equirectangular(num_rays, 1, image_height, image_width, device=device)
                else:
                    # if random_num >= 0.8:
                    #     indices = self.sample_method(num_rays, 1, image_height, image_width, device=device, step=step, image=batch["image"][i])
                    # else:
                    # indices = self.sample_method(num_rays, 1, image_height, image_width, device=device, step=step, image=None, patch=patch)
                    indices = self.sample_method(num_rays, 1, image_height, image_width, device=device, step=step)
                # print(indices.shape)
                # import cv2
                # iamge = batch["image"][i].detach().cpu().numpy() * 255
                # cv2.imwrite("./test.jpg", iamge)
                # import sys;sys.exit()
                indices[:, 0] = i
                all_indices.append(indices)
                all_images.append(batch["image"][i][indices[:, 1], indices[:, 2]])
        ...
        collated_batch["mask"] = torch.cat(all_maskes, dim=0)
        if "depth_image" in batch:
            collated_batch["depth_image"] = torch.cat(all_depth_images, dim=0)
        # print("collated_batch['image'].shape", collated_batch["image"].shape, num_rays_per_batch)
        # print(num_rays_per_batch)
        assert collated_batch["image"].shape[0] == num_rays_per_batch
    ...
    def sample(self, image_batch: Dict, step: int=None):
        ...
        if isinstance(image_batch["image"], list):
            ...
            pixel_batch = self.collate_image_dataset_batch_list(
                image_batch, self.num_rays_per_batch, keep_full_image=self.config.keep_full_image, step=step
            )
