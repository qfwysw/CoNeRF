...
class DepthRenderer(nn.Module):
    ...
    def forward(
        ...
        if self.method == "median":
            steps = (ray_samples.frustums.starts + ray_samples.frustums.ends) / 2  # [..., num_samples, 1]

            if ray_indices is not None and num_rays is not None:
                raise NotImplementedError("Combined depth calculation is not implemented for packed samples.")

            # --- Max method ---
            max_weight_index = torch.argmax(weights[..., 0], dim=-1, keepdim=True)  # [..., 1]
            max_weight_depth = torch.gather(steps[..., 0], dim=-1, index=max_weight_index)  # [..., 1]
            ...
            # 判断 max 和 median 的深度是否接近
            epsilon = 1e-3  # 可根据需要调整
            close_mask = (torch.abs(max_weight_depth - median_depth) < epsilon).float()  # 1 表示接近
            # res = torch.abs(max_weight_depth - median_depth)
            # print(res.min(), res.max())
            # 如果接近则使用 max，否则使用 median
            combined_depth = close_mask * max_weight_depth + (1.0 - close_mask) * median_depth  # [..., 1]
        ...
        #     if ray_indices is not None and num_rays is not None:
        #         raise NotImplementedError("Max-weight depth calculation is not implemented for packed samples.")

        #     # 找到权重最大的索引
        #     max_weight_index = torch.argmax(weights[..., 0], dim=-1, keepdim=True)  # [..., 1]

        #     # 从 steps 中根据最大权重索引提取对应的深度
        #     max_weight_depth = torch.gather(steps[..., 0], dim=-1, index=max_weight_index)  # [..., 1]

        #     return max_weight_depth
