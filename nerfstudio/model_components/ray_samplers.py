...
class CenterWeightedSampler(SpacedSampler):
    """
    A piecewise sampler that focuses more samples around the center of the ray (in normalized depth space).

    Args:
        num_samples: Number of samples per ray
        train_stratified: Use stratified sampling during training
        single_jitter: Use the same random jitter for all samples along a ray
        sharpness: Controls how concentrated the samples are around the center (higher = more concentrated)
    """
    ...
    def __init__(
        ...
        train_stratified=True,
        single_jitter=False,
        sharpness=5.0,  # σ in exp(-((x-0.5)^2)/σ)
    ) -> None:
        self.sharpness = sharpness

        # Define mapping function: input x in [0, 1], output biased towards center
        def spacing_fn(x):
            # Normalize Gaussian bell curve to [0, 1]
            center_bias = torch.exp(-((x - 0.5) ** 2) * sharpness)
            cumsum = torch.cumsum(center_bias, dim=-1)
            return cumsum / cumsum[..., -1:]  # normalize to [0, 1]

        # Inverse spacing is complex analytically, approximate with identity (or use numerical inverse if needed)
        def spacing_fn_inv(x):
            return x  # fallback to uniform inverse

        super().__init__(
            num_samples=num_samples,
            spacing_fn=spacing_fn,
            spacing_fn_inv=spacing_fn_inv,
            train_stratified=train_stratified,
            single_jitter=single_jitter,
        )
...
#     """Sample based on probability distribution with amplification of top weights.

#     Args:
#         num_samples: Number of samples per ray.
#         train_stratified: Randomize location within each bin during training.
#         single_jitter: Use the same random jitter for all samples along a ray.
#         include_original: Include original samples in the returned results.
#         histogram_padding: Small value added to weights to avoid zero-sum.
#         amplification_factor: Factor to amplify the top weights (default: 2.0).
#     """
...
#         if self.training:
#             with torch.no_grad():
#                 k = min(1, weights.shape[-1])
#                 if k > 0:
#                     top_indices = torch.topk(weights, k=k, dim=-1, largest=True, sorted=False).indices
#                     values = torch.gather(weights, -1, top_indices)
#                     amplified_values = values * self.amplification_factor
#                     weights.scatter_(-1, top_indices, amplified_values)
...
#             else:
#                 rand = torch.rand((*cdf.shape[:-1], num_bins), device=cdf.device) / num_bins
#             u = u + rand
#         else:
#             u = torch.linspace(0.0, 1.0 - (1.0 / num_bins), steps=num_bins, device=cdf.device)
#             u = u + 1.0 / (2 * num_bins)
...
#             [ray_samples.spacing_starts[..., 0], ray_samples.spacing_ends[..., -1:, 0]],
#             dim=-1,
#         )
...
#         bins_below = torch.gather(existing_bins, -1, below)
#         bins_above = torch.gather(existing_bins, -1, above)

#         denom = (cdf_above - cdf_below)
#         denom = torch.where(denom < eps, torch.ones_like(denom), denom)
#         t = (u - cdf_below) / denom
#         bins = bins_below + t * (bins_above - bins_below)

#         if self.include_original:
#             bins, _ = torch.sort(torch.cat([existing_bins, bins], dim=-1), dim=-1)
...
class PDFSampler(Sampler):
    """Sample based on probability distribution with optional dropout regularization.
    ...
    Args:
        ...
        include_original: Include original samples in the returned results.
        histogram_padding: Small value added to weights to avoid zero-sum.
        dropout_rate: Dropout rate applied to PDF weights during training.
        use_soft_dropout: If True, apply soft dropout instead of hard masking.
        max_epoch: Optional max epoch for decay schedule.
    """
    ...
    def __init__(
        ...
        self.use_soft_dropout = use_soft_dropout
        self.max_epoch = 60000
        self.current_epoch = 0  # 可由外部设置
        

    def set_epoch(self, epoch: int):
        self.current_epoch = epoch

    def get_effective_dropout_rate(self):
        # 初始高扰动，逐步衰减至0
        if self.max_epoch is not None and self.max_epoch > 0:
            # print(self.max_epoch, self.current_epoch)
            return self.dropout_rate * (1 - self.current_epoch / 60000)
    ...
    def generate_ray_samples(
        ...
        if self.training:
            if self.current_epoch >= 0:
                # --- Dropout 部分 ---
                # print("Dropout")
                effective_dropout = self.get_effective_dropout_rate()
                # effective_dropout = 0
                if effective_dropout > 0.0 and self.training:
                    if self.use_soft_dropout:
                        # Soft Dropout：扰动权重
                        # noise = 2 * torch.rand_like(weights) - 1  # 范围[-1,1)
                        # dropout_mask = 1.0 + noise * (effective_dropout / 2)
                        # weights *= dropout_mask.clamp(min=1 - effective_dropout, max=1 + effective_dropout)
                        ###############
                        # 根据权重大小调整噪声强度
                        # print("dropout")
                        weights_copy = weights.clone().detach()
                        normalized_weights = weights_copy / (weights_copy.max(dim=-1, keepdim=True).values + 1e-8)  # 归一化权重 [0,1]
                        noise_scale = 1.0 - normalized_weights  # 高权重 → 小扰动，低权重 → 大扰动
                        noise = torch.rand_like(weights) * noise_scale  # 噪声强度与权重成反比
                        dropout_mask = 1.0 - noise * effective_dropout  # Soft Dropout
                        # import pdb;pdb.set_trace()
                        weights *= dropout_mask
                        ###############
                        # Soft Dropout：高权重抑制 + 邻域延申
                        # weights_copy = weights.clone().detach()

                        # # Step 1: 归一化权重 [0,1]
                        # normalized_weights = weights_copy / (weights_copy.max(dim=-1, keepdim=True).values + 1e-8)

                        # # Step 2: 高权重区域抑制（使用平方放大高权重）
                        # suppression_strength = normalized_weights ** 2  # 越接近1，扰动越大

                        # # Step 3: 构造扰动掩码（1 - 抑制 * dropout 强度）
                        # dropout_mask = 1.0 - suppression_strength * effective_dropout

                        # # Step 4: 向左右扩散扰动 (1D 卷积方式)
                        # # 卷积核: [0.25, 0.5, 0.25] 扩散扰动效果
                        # kernel = torch.tensor([0.25, 0.5, 0.25], device=weights.device).view(1, 1, 3)
                        # dropout_mask = dropout_mask.unsqueeze(1)  # [B, 1, N]
                        # dropout_mask = F.pad(dropout_mask, (1, 1), mode='replicate')  # padding for conv
                        # dropout_mask = F.conv1d(dropout_mask, kernel)  # [B, 1, N]
                        # dropout_mask = dropout_mask.squeeze(1)  # [B, N]
                        # # import pdb;pdb.set_trace()
                        # # Step 5: 应用扰动
                        # weights *= dropout_mask
                    else:
                        # Hard Dropout：启发式遮盖部分区域
                        mask = torch.rand_like(weights) >= effective_dropout
                        weights = weights * mask.float() / (1.0 - effective_dropout)
            else:
                if self.training:
                    # print("topk")
                    with torch.no_grad():
