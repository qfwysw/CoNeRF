...
def interlevel_loss(weights_list, ray_samples_list, mask) -> torch.Tensor:
    ...
    for ray_samples, weights in zip(ray_samples_list[:-1], weights_list[:-1]):
        ...
        # print(c.shape, w.shape, cp.shape, wp.shape)
        # import pdb;pdb.set_trace()
        # wp = wp * mask
        # cp = cp * mask
        loss_interlevel += torch.mean(lossfun_outer(c, w, cp, wp) * mask)
